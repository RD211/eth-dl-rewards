...
def generate_sft_data(cfg: DataGenerationConfig) -> None:
  ...
  for idx in tqdm(range(0, len(dataset['messages']), cfg.generation.problems_per_batch), desc="Generating preferences"):
    ...
    for i in range(len(problems)):
      ...
      for response in responses:
        sft_data.append((problem, response))
    ...
    if idx % cfg.logging.save_every == 0 or idx >= len(dataset['messages']) - cfg.generation.problems_per_batch:
      ...
      transformed_sft_data = []
      for problem, completion in sft_data:
        transformed_sft_data.append({
          "problem": problem,
          "completion": completion
        })

      # 3 Columns problem, accepted, rejected
      df = pd.DataFrame(transformed_sft_data)
